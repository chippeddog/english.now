...
function PricingPage() {
	...
	const renderPlanCard = (_plan: (typeof _plans)[number]) => {
		const features = t(`pricing.plans.${_plan.key}.features`, {
			returnObjects: true,
		}) as string[];
		return (
			<div
			...
			>
				<div className="relative z-10 mb-4">
					<div className="flex flex-col gap-1.5">
						...
						<div className="text-muted-foreground text-sm">
							{t(`pricing.plans.${_plan.key}.description`)}
						</div>
					</div>
				</div>
				<div className="relative z-10 flex items-baseline">
					...
					<div className="ml-1 text-muted-foreground text-sm md:text-sm md:leading-7">
						/{t(`pricing.duration.${_plan.duration}`)}
					</div>
				</div>
	...
	const renderPopularWrapper = (
		...
		<div
			className={cn("rounded-[1.6rem] p-2", extraClassName)}
			style={{
				background:
				...
				<div className="mx-auto max-w-3xl sm:mt-20">
					<div className="mb-10 text-center">
						<h2 className="mb-2 font-bold font-lyon text-5xl tracking-tight md:text-5xl">
							{t("pricing.faqSection.title")}
						</h2>
						<p className="text-lg text-muted-foreground">
							{t("pricing.faqSection.subtitle")}
						</p>
					</div>
					...
					<div className="overflow-hidden rounded-3xl bg-neutral-100 p-2">
						...
						>
							{PRICING_FAQ_KEYS.map((faqKey, index) => (
								<AccordionItem
									key={faqKey}
									value={`item-${index}`}
									className="px-6"
								>
									<AccordionTrigger className="py-5 text-left font-medium hover:no-underline">
										{t(`pricing.faqSection.items.${faqKey}.question`)}
									</AccordionTrigger>
									<AccordionContent className="pb-5 text-muted-foreground transition-all duration-300">
										{t(`pricing.faqSection.items.${faqKey}.answer`)}
									</AccordionContent>
								</AccordionItem>
							))}
						</Accordion>
