...
async function main() {
	const { level, count, category, linkLessons } = parseArgs();
	console.log(`Generating ${count} canonical grammar topics for ${level}...`);
	...
	const plan = await generateTopicPlan(level, count, category, existingSlugs);
